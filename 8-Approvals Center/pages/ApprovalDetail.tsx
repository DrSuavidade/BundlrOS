...
import { ArrowLeft, Clock, Paperclip, Share2, MessageSquare, Send, Sparkles, Loader2 } from 'lucide-react';
...
export const ApprovalDetail: React.FC = () => {
  ...
  const openDraft = async (type: 'APPROVE' | 'REJECT') => {
      ...
      const text = await GeminiService.draftResponse(type, approval?.title + " - " + approval?.description || "");
  ...
  const handleDecision = async () => {
      if(!approval) return;
      setLoading(true);
      await ApprovalService.updateStatus(approval.id, draftType === 'APPROVE' ? ApprovalStatus.APPROVED : ApprovalStatus.REJECTED, draftContent, 'Admin');
      const updated = await ApprovalService.getById(approval.id);
      if(updated) setApproval(updated);
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
            <button onClick={handleCopyLink} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50 text-sm font-medium">
                <Share2 size={16} /> Share Link
            </button>
            {approval.status === ApprovalStatus.PENDING && (
                <button onClick={handleSendReminder} className="px-4 py-2 bg-white border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50 text-sm font-medium">
                    Send Reminder
                </button>
            )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-8">
                <div className="flex justify-between items-start mb-6">
                    <div>
                        ...
                        <h1 className="text-3xl font-bold text-slate-800">{approval.title}</h1>
                    </div>
                    {approval.status === ApprovalStatus.PENDING && (
                        <div className="flex gap-3">
                             <button onClick={() => openDraft('REJECT')} className="px-4 py-2 rounded-lg border border-red-200 text-red-700 bg-red-50 hover:bg-red-100 font-medium text-sm">
                                Reject
                             </button>
                             <button onClick={() => openDraft('APPROVE')} className="px-4 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 font-medium text-sm shadow-sm shadow-indigo-200">
                                Approve
                             </button>
                        </div>
                    )}
                ...
                <div className="prose prose-slate max-w-none">
                    ...
                    {summary && (
                         <div className="mt-4 bg-indigo-50 p-4 rounded-lg border border-indigo-100 flex gap-3">
                             ...
                             <div>
                                 <p className="text-xs font-bold text-indigo-700 uppercase mb-1">AI Summary</p>
                ...
                <div className="mt-8 pt-8 border-t border-slate-100">
                    <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Attachments</h3>
                    {approval.attachmentName ? (
                        <div className="flex items-center gap-3 p-3 border border-slate-200 rounded-lg w-fit hover:border-indigo-300 hover:bg-indigo-50 transition-colors cursor-pointer">
                            ...
                            <div className="pr-4">
                                <p className="text-sm font-medium text-slate-700">{approval.attachmentName}</p>
        ...
        <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
                <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Request Details</h3>
                <div className="space-y-4">
                    <div className="flex justify-between text-sm">
                        <span className="text-slate-500">Client</span>
                        <span className="font-medium text-slate-800">{approval.clientName}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                        <span className="text-slate-500">Requested</span>
                        <span className="font-medium text-slate-800">{format(new Date(approval.createdAt), 'MMM d, yyyy')}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                        <span className="text-slate-500">Due Date</span>
                        <span className="font-medium text-slate-800">{format(new Date(approval.dueDate), 'MMM d, yyyy')}</span>
                    </div>
                </div>
            </div>
                    ...
                    {approval.history.map((event) => (
                        <div key={event.id} className="relative pl-8">
                            <div className={`absolute left-0 top-1 w-4 h-4 rounded-full border-2 border-white ${
                                event.type === 'STATUS_CHANGED' ? 'bg-indigo-500' : 'bg-slate-300'
                            }`}></div>
                            <p className="text-sm text-slate-800 font-medium">{event.description}</p>
                            <p className="text-xs text-slate-400 mt-1">
                                {format(new Date(event.timestamp), 'MMM d, h:mm a')} • {event.actor}
                            </p>
                        </div>
                    ))}
                </div>
        ...
        {aiDraftOpen && (
            <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
                    <div className={`px-6 py-4 border-b ${draftType === 'APPROVE' ? 'bg-indigo-50 border-indigo-100' : 'bg-red-50 border-red-100'} flex justify-between items-center`}>
                        <div className="flex items-center gap-2">
                            <Sparkles size={18} className={draftType === 'APPROVE' ? 'text-indigo-600' : 'text-red-600'} />
                            <h3 className={`font-semibold ${draftType === 'APPROVE' ? 'text-indigo-800' : 'text-red-800'}`}>
                                AI Draft Assistant
                            </h3>
                        </div>
                        <button onClick={() => setAiDraftOpen(false)} className="text-slate-400 hover:text-slate-600">&times;</button>
                    </div>
                    <div className="p-6">
                        <p className="text-sm text-slate-500 mb-2">Review and edit the {draftType.toLowerCase()} message before sending:</p>
                        ...
                        <div className="mt-4 flex justify-end gap-2">
                             <button onClick={() => setAiDraftOpen(false)} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg text-sm font-medium">Cancel</button>
                             <button 
                                onClick={handleDecision} 
                                disabled={drafting}
                                className={`px-4 py-2 rounded-lg text-white text-sm font-medium ${draftType === 'APPROVE' ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-red-600 hover:bg-red-700'}`}
                             >
