...
import { MoreVertical, Search, Plus, X, Edit2, Ban, CheckCircle } from 'lucide-react';
...
export const UsersList: React.FC = () => {
  ...
  const handleCreateOrUpdate = (e: React.FormEvent) => {
    ...
    setEditingUser(null);
    setFormData({ name: '', email: '', role: Role.DEV, organizationId: 'org_main' });
    refreshUsers();
  };
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-white/5 bg-white/5">
                ...
                <th className="px-6 py-4 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Organization</th>
                ...
                <tr key={user.id} className="hover:bg-white/5 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openEdit(user)} className="p-1.5 text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700 rounded-md">
                        <Edit2 size={16} />
                      </button>
                      <button onClick={() => toggleStatus(user)} className="p-1.5 text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700 rounded-md">
                        {user.status === 'active' ? <Ban size={16} className="text-red-400" /> : <CheckCircle size={16} className="text-green-400" />}
                      </button>
                    </div>
                  </td>
                </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <GlassCard className="w-full max-w-md animate-fade-in-up">
             ...
             <form onSubmit={handleCreateOrUpdate} className="space-y-4">
                ...
                <div>
                    <label className="block text-sm font-medium text-zinc-400 mb-1">Organization ID</label>
                    <input 
                        required
                        type="text" 
                ...
                <div className="pt-4 flex justify-end gap-3">
                    ...
                    <button type="submit" className="px-4 py-2 bg-white text-black text-sm font-medium rounded-lg hover:bg-zinc-200">
